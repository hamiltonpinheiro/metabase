...
#_(defn- query+metadata->hash [query metadata]
  (codec/base64-encode (hash/md5 (str (json/generate-string query)
...
#_(let [results  (qp query)
        metadata (results->column-metadata results)]
    (assoc results
      :metadata metadata
      :checksum (query+metadata->hash query metadata)))
